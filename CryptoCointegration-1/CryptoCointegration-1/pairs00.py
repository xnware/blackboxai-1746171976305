...
# Настройка логгера для модуля работы с парами торговых инструментов
logger = logging.getLogger(__name__)

def fetch_pairs(start_date: str, num_pairs: int, sort_by: str, periods: int = 100) -> List[Dict[str, Any]]:
    """
    Загрузка коинтегрированных пар и их данных
    
    Эта функция получает из базы данных список коинтегрированных пар,
    загружает данные по спредам и рассчитывает индикаторы.
    
    Аргументы:
        start_date: Начальная дата для данных (в формате YYYY-MM-DD)
        num_pairs: Количество пар для возврата
        sort_by: Метод сортировки (z_score,
